...
test('6. Feature Extraction Determinism', async (t) => {
  ...
  await t.test('Graph feature extraction produces identical output on repeated runs', () => {
    const neighborsA = ['node_x', 'node_y', 'node_z'];
    const neighborsB = ['node_y', 'node_z', 'node_w'];

    const run1 = extractGraphFeatures(neighborsA, neighborsB);
    const run2 = extractGraphFeatures(neighborsA, neighborsB);

    assert.deepEqual(run1, run2, 'Graph features must be strictly deterministic');
    assert.equal(run1.commonNeighborsCount, 2);
    assert.equal(run1.jaccardSimilarity, 0.5);
  });
});
